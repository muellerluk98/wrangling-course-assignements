...
this chunk is written so that in the end the whole code can be knitted and put into an html file
```{r, include=FALSE}
...
# Dependencies
Here we are loading all necessary packages
```{r}
...
Usually you get a short description, informations about the syntax and usage, information about the arguments/elements this function requires. You can also get informations about whole packages like dplyr.
...
- Why use {reader}'s `read_csv()` over the base R `read.csv()`? 

Because `read_csv()` is more explicit about what assumption it is making about column types, and prints warning messages about what it has assumed.
...
The function getwd() shows you an absolute filepath which leads to your current working directory while the function setwd() is designed to set a new working directory. It is better to never change working directories so that the code also runs on other machines. It is better to save stuff on GitHub and then using relative instead of absolute paths so that also other people can work on the code. If someone sets a new working directory some informations/data may be lost because maybe on other machines there isn't the whole data in this ''new'' working directory
...
This chunk loads the data with a relative path: The relative path explained:
.. = go one level back
data = select folder data
raw = select folder raw
data_demographics_raw.csv = choose this file
...
To answer this questions i would first look at the dataframes when i loaded them in R. There we see:
data_amp_raw has 8224 observations of 10 variables
data_demographics_raw has 200 observations of 17 variables
data_selfreport_raw has 392 observations of 17 variables
additionally we recognised that every dataframe got a subject column.
so the answer is that there are different amounts of rows in the dataframes because the number of rows per participant differs in all dataframes.

- Why are the numbers not round?

The numbers are round hahah?! --> They aren´t round in the sense that they aren´t a multiplicated version of each other. That´s because there are some missing values for some participants.

# The pipe (%>% or %>%) --> shouldn´t there be this sign |>
...
`%>%` is a version of the pipe more recently added to base-R. It is slightly faster but less flexible. --> shouldn´t there be this sign |>

If you're not sure, it's easier to use `%>%`. 

## What is the pipe?

The pipe is a very useful explanation in R because you can tell the programme to take the output of one codeline as the input for the upcoming line.
...
We use the pipe to make our code look way more smoother. By using the pipe we can keep the code compact and we can make one order with many steps.
...
Here we see that the pipe gives us a way more beautiful and understandable embodiement of our code. that will also help you to reduce errors because it's way more clearer
...
- i would simply try to write ?clean_names or ?janitor or ?janitor::clean_names()
?clean_names
?janitor::clean_names()
?janitor
...
- we use select for columns and filter for rows
...
round(1.5) # should be 2, bankers rounding will give us 2
round(2.5) # should be 3, bankers rounding will give us 2
round(3.5) # should be 4, bankers rounding will give us 4
round(4.5) # should be 5, bankers rounding will give us 4
round(5.5) # should be 6, bankers rounding will give us 6

```

Remember: you probably need to use `janitor::round_half_up()` in most of your R scripts --> that will give us correct rounding as we learnt it in school
...
data_selfreport_trimmed %>% 
  summarize(mean(selfreport)) %>% 
  summarize(sd(selfreport))
